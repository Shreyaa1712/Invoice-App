...
export default function Invoice() {
  ...
  const saveToFirestore = async () => {
    try {
      ...
      const docRef = doc(db, "invoice2", timeRef); // Use a unique identifier (invoiceNumber) for the document ID
      await setDoc(docRef, {
        email: emailid,
        name: invoicename,
        data: invoiceData,id:timestamp,
        type:type
      });

      alert("Success", "Invoice data saved successfully!");
    } catch (error) {
